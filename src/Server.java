...
public class Server {
	...
	String[] getLocalAddresses() {
		try {
			...
			String[] result = new String[addresses.size()-1];
			int index = 0;
	...
	private boolean receiveFileChunks(String shortFile, String newFileName, File selectedFile, InputStream in)
			throws IOException {
	...
	private boolean receiveFile(String shortFile, String newFileName, File selectedFile, InputStream in)
			throws IOException {
		File targetFile = null;
		try {
			// get 4 byte length of the file
	...
	private String[] getListOfAvailableFiles(int numFiles, InputStream in, String rootFolder, File localDirectory)
			throws IOException {
	...
	private void receiveFiles() {
		receiveThread = new Thread(() -> {
			...
			File selectedFile = new File("RECEIVE/");
			selectedFile.mkdir();
